    ...
    if CFG.model.name=="dino_vit_B":
        ...
        def vit_small(pretrained, progress, key, **kwargs):
            ...
            model = VisionTransformer(
                img_size=224, patch_size=patch_size, embed_dim=384, num_heads=6, num_classes=0,dynamic_img_size=True
            )
            if pretrained:
...
def main(CFG : DictConfig) -> None:
    ...
    for SEED in [0,1,2,3,4]:
        ...
        for fold in range(folds["fold"].nunique()):
            ...
            _preds, _valid_labels,_oof_val = train_fn(SEED,CFG,fold,folds)
            preds.append(_preds)
        ...
        oof.to_csv(f"oof-{SEED}-_{CFG.general.exp_num}.csv",index=False)

        test_df,pred_ = submit(test_df,folds,CFG)
        test_df.to_csv(f"sub-{SEED}_{CFG.general.exp_num}.csv",index=False)
