...
def sel(df,SEED=42):
    ...
    for label in df["label"].unique():
        ...
        if tmp["WSI"].nunique()==1:
            tmp = tmp.sample(frac=0.1,random_state=SEED)
        else:
            tmp_WSI = list(tmp["WSI"].unique())
            tmp_WSI = np.random.choice(tmp_WSI, size=len(tmp_WSI)//10, replace=False)
...
def eval_linear(args):
    ...
    #tra_df = sel(tra_df)    
    tra_df = sel(tra_df,SEED=int(args.SEED))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', default="/home/abe/KidneyM/dino/FIX/4cls_L600__eGFR_over", help='Path to save logs and checkpoints')
    parser.add_argument('--num_labels', default=2, type=int, help='Number of labels for linear classifier')
    parser.add_argument('--SEED', default=42, type=int, help='Number of labels for linear classifier')
    ...
    for SEE_D in [0,1,2,3,4]:
        seed_torch(seed=SEE_D)
        args.SEED=SEE_D
        args.fold = 0
        ROOT = f"{args.output_dir}_{args.COL}_over_{args.CONF}__SEED{args.SEED}/"
