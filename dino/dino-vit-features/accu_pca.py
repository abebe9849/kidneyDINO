...
def pca(image_paths, load_size: int = 224, layer: int = 11, facet: str = 'key', bin: bool = False, stride: int = 4,
    ...
    for idx in tqdm(range(5)):
        with open(f'/home/abe/KidneyM/dino/dino-vit-features/accum_data/image_pil_list_{idx}.pickle', 'rb') as f:
            image_pil_list += pickle.load(f)
        with open(f'/home/abe/KidneyM/dino/dino-vit-features/accum_data/num_patches_list_{idx}.pickle', 'rb') as f:
            num_patches_list += pickle.load(f)
        with open(f'/home/abe/KidneyM/dino/dino-vit-features/accum_data/load_size_list_{idx}.pickle', 'rb') as f:
            load_size_list += pickle.load(f)
        with open(f'/home/abe/KidneyM/dino/dino-vit-features/accum_data/descriptors_list_{idx}.pickle', 'rb') as f:
            descriptors_list += pickle.load(f)
    #if all_together:
    descriptors = np.concatenate(descriptors_list[:10], axis=2)[0, 0]
    print(descriptors.shape,len(descriptors_list))
    pca_descriptors = PCA(n_components=n_components).fit_transform(descriptors)#どの画像のlistを入れたのかによって結果変わる
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_dir',default="/home/abe/KidneyM/dino/dino-vit-features/size512_all", type=str, help='The root save dir for results.') #
