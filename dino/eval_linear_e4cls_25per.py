...
all_df = all_df[all_df["label"]<4].reset_index(drop=True)
...
for i in range(n_target):
    tmp  = all_df[all_df["label"]==i].reset_index(drop=True)
    print(tmp.shape)
    sgkf = StratifiedGroupKFold(n_splits=N_fold,random_state=2025,shuffle=True)
    for fold, ( _, val_) in enumerate(sgkf.split(X=tmp, y=tmp.label.to_numpy(),groups=tmp.WSI)):
        tmp.loc[val_ , "fold"] = fold
        
        val_df = tmp[tmp["fold"]==fold]
...
def eval_linear(args):
    ...
    SEED = int(args.seed25)
    tra_df = sel(tra_df,SEED)
