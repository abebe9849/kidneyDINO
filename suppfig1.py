...
all_df = all_df[all_df["kouka_or_not"]==True].reset_index(drop=True)
print(all_df.shape)
...
all_df = all_df[all_df["label"].isin([0,1,2,3])].reset_index(drop=True)
print(all_df.shape)
...
def train_fn(CFG,fold,folds):
    ...
    if CFG.model.name=="dino_vit_B":
        ...
        pretrained_weights = "/home/abe/KidneyM/dino/pas_glomerulus_kouka/checkpoint.pth"
...
def submit(test_df,folds,CFG):
    ...
    for fold in range(folds["fold"].nunique()):
        if CFG.model.name=="dino_vit_B":
            ...
            model.head = nn.Linear(768*CFG.model.Ncat,4)
        elif "vit_base" in CFG.model.name or "vit_large" in CFG.model.name:
            model = timm.create_model(CFG.model.name, pretrained=True,num_classes=4,in_chans=3,img_size=CFG.preprocess.size)
        else:
            model = timm.create_model(CFG.model.name, pretrained=True,num_classes=4,in_chans=3) 
        model.load_state_dict(torch.load(f'fold{fold}_{CFG.general.exp_num}_best_AUC.pth'))
...
def main(CFG : DictConfig) -> None:
    ...
    AUC_score = roc_auc_score(test_df["label"], pred_,multi_class="ovr")
    log.info(f"TEST")
    log.info(f"AUC_mean :{AUC_score}")
